...
const RagSystem = () => {
  ...
  const handleFileUpload = (event) => {
    ...
    axios.post('http://localhost:8000/api/rag/processdocuments', formData, {
      headers: {
      ...
      .catch(err => {
        ...
        setIsLoading(false);
        setIsThinking(false);
        setMessages(prevMessages => [...prevMessages, { text: `Error processing documents: ${err.response?.data?.detail || err.message}`, isUser: false }]);
      });
  };
  ...
  const handleSubmit = () => {
    ...
    axios
      ...
      .then((res) => {
        setIsThinking(false);
        const responseText = activeTab === "RAG Chat" ? res.data.answer : res.data.response;
        setMessages((prevMessages) => [
          ...prevMessages,
  ...
  const handleUrlSubmit = () => {
    ...
    axios
      .post("http://localhost:8000/api/rag/processdocuments", formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      })
      .then((res) => {
        ...
        setMessages((prevMessages) => [
          ...prevMessages,
          { text: "URL content processed successfully!", isUser: false },
        ]);
      })
      .catch((err) => {
        console.error(err);
        ...
        setMessages((prevMessages) => [
          ...prevMessages,
          { text: "Error processing the URL content.", isUser: false },
        ]);
      });
  ...
  return (
    <div
      className="min-h-screen w-full flex flex-col items-center bg-cover bg-center relative"
          ...
          <div className="bg-white bg-opacity-90 p-8 rounded-lg shadow-lg w-full">
            ...
            <div className="flex items-center">
              ...
              <button
                ...
                className="bg-blue-500 text-white p-2 rounded-r-lg transition-colors hover:bg-blue-600"
              >
                <Send size={20} />
              </button>
            </div>
        ...
        <Fade>
          <div className="flex justify-between items-center max-w-4xl mx-auto">
            <div className="flex items-center space-x-4">
              <label
                htmlFor="file-upload"
                className="cursor-pointer flex items-center space-x-2 bg-blue-500 text-white px-4 py-2 rounded-lg transition-colors hover:bg-blue-600"
              >
                <Upload size={20} />
